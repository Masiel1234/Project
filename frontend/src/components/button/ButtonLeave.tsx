    ...
    <>
      <button
       onClick={handleAbandon}
        className="fixed top-4 left-4 w-30 h-12 bg-pink-600 rounded-full flex items-center justify-center text-white shadow-lg hover:bg-pink-700 transition-all duration-300 ease-in-out"
      >
       {t('buttonLeave.title')}
      </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-blue-100/70 bg-opacity-100 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full mx-4">
            <p className="text-lg font-semibold text-black mb-6 text-center">{t('buttonLeave.message')}</p>
            <div className="flex justify-around space-x-4">
              ...
              >
                {t('buttonLeave.confirmButtonText')}
              </button>
              <button
              ...
              >
                {t('buttonLeave.cancelButtonText')}
              </button>
            </div>
          </div>
        </div>
